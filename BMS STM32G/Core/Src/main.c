...
void LTC6804_ReadCellVoltages(uint16_t *voltages) {
    ...
    for(int i = 0;i<4;i++){
    	uint8_t Temp_Voltage_Buffer[6]; //Temporary Buffer Which Obtains Read Value and reset every iteration
    	LTC6804_ReadData_DMA(cmd[i], Temp_Voltage_Buffer, 6); //Function to read SPI Data through DMA
    	int count = 0; //This varies 0 to 3 in the below loop every time
    	//To make 2x8bit data into a 16bit
    	for(int j = 3*i;j<(3*i)+3;j++){
    		voltages[j] = (Temp_Voltage_Buffer[2*count] | (Temp_Voltage_Buffer[(2*count)+1]));
    		count++;
    	}
    }
}
...
void SystemClock_Config(void)
{
  ...
  */
  ...
  RCC_OscInitStruct.PLL.PLLState = RCC_-PLL_ON;
